...
export function runVerify(root?: string): {
  ...
  for (const file of FILE_TRANSFORMS) {
    ...
    try {
      if (file.transform(source) !== source) {
        issues.push(`${file.path} missing agenthosts call sites`);
      }
    } catch (error) {
      issues.push(
        `${file.path} has invalid agenthosts call sites: ${
  ...
  for (const rule of HOST_COPY_RULES) {
    if (!fs.existsSync(path.join(nanoclawRoot, rule.dest)))
      issues.push(`missing ${rule.dest}`);
  }
...
export function syncSkillToFork(
  ...
  const destination = path.join(nanoclawRoot, ".claude/skills/add-agenthosts");
...
export function printInstallNextSteps(
  ...
  console.log(
    `${options.upgraded ? "Upgraded" : "Installed"} nanoclaw-agenthosts@${result.version} into ${result.root}`,
  );
  console.log(
  ...
  console.log("  2. pnpm exec nanoclaw-agenthosts verify");
  console.log("  3. Restart the NanoClaw host service.");
  console.log(
    "  4. (Optional) install agenthost plugins (process / applenative / flyio).",
  );
  console.log(
    "  5. Select a runtime per group: ncl groups config update --id <ag> --runtime process",
  );
}
...
function atomicWrite(target: string, content: Buffer, mode?: number): void {
  ...
  const temporary = `${target}.agenthosts-${process.pid}-${randomBytes(4).toString("hex")}.tmp`;
  try {
    fs.writeFileSync(
      temporary,
      content,
