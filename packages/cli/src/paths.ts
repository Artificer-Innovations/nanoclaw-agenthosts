...
export const HOST_COPY_RULES: CopyRule[] = [
  ...
  { source: "agenthosts.ts", dest: "src/agenthosts.ts" },
  {
    source: "020-agenthosts-runtime.ts",
    dest: "src/db/migrations/020-agenthosts-runtime.ts",
  },
];
...
export function packageRoot(startDir: string = __dirname): string {
  ...
  for (;;) {
    ...
    if (fs.existsSync(packagePath)) {
      try {
        ...
        if (pkg.name === "nanoclaw-agenthosts") return dir;
      } catch {
        // continue walking up
      }
    }
  ...
  throw new Error("Could not locate nanoclaw-agenthosts package root");
}

export function skillDir(startDir: string = __dirname): string {
  return path.join(packageRoot(startDir), "skills/add-agenthosts");
}
...
export function hostResourcesDir(startDir: string = __dirname): string {
  ...
  if (fs.existsSync(path.join(source, "agenthosts.ts"))) return source;
